...
for i = size(rawValues,2):-1:1
    for j = 1:numStates
        for k = 1:numStates
            ...
            indsY(k) = i+max(1,round(slidertime/dt*min(k,numStates-k)));
...
while i <= size(traj,2)
    ...
    if acta == numStates
        ...
        if acta < numStates-acta
            traj(iOld:i) = mod(linspace(traj(iOld-1),traj(iOld-1)-acta,slideTime)-.5,numStates)+.5;
        else
            traj(iOld:i) = mod(linspace(traj(iOld-1),traj(iOld-1)+numStates-acta,slideTime)-.5,numStates)+.5;
        end
    end
    i = i+1;
end
