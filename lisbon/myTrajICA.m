function [wx Xor trialNum stimOr] = myTrajICA(fn,ILD)
...
for i = 2:size(data,1)%1:size(d,1)
    ...
    if version == 1
        trajcwt = [cwt(real(diff(traj,[],2)),scales,'cgau1'); cwt(imag(diff(traj,[],2)),scales,'cgau2')];
        trajcwt = [zeros(size(trajcwt,1),1) trajcwt];
    elseif version == 2
            trajcwt = [cwt(real(traj),scales,'cgau1'); cwt(imag(traj),scales,'cgau2')];
    else
    ...
    stimcwt = cwt(data{i,6}(~isnan(data{i,6})),scales,'cgau1');
    X = [X trajcwt];%(:,f)];
    Xor = [Xor traj(:,~isnan(data{i,6}))];%(f)];
    stim = [stim stimcwt];
    stimOr = [stimOr data{i,6}(~isnan(data{i,6}))];
    %dx = squeeze(d(i,f,1));
    %dy = squeeze(d(i,f,2));
...
[A W Z] = cfastica(X);%,'mle_circ');%
wx = W*X;
ang = angle(mean(sign(real(wx)).*wx,2)).';
%ang = angle(mean(A(1:end/2,:)));
A = bsxfun(@times,A,exp(1i*ang));W = bsxfun(@times,W,exp(1i*-ang).');wx = bsxfun(@times,wx,exp(1i*-ang).');
end
...
%return
figure;for i = 1:numel(trialNum)-1
    inds = trialNum(i)+1:trialNum(i+1);
    for j = 1:min(25,size(wx,1))
        %acc1 = abs(W(j,:)*X(:,inds));acc1 = [max(1,min(ceil(acc1/max(acc1)*64),64))];
        act = wx(j,inds);
        cols = squeeze(complexIm(act,0,.5));indPlot = abs(act) > 1;
        subplot(5,5,j);scatter(real(Xor(inds(indPlot))),imag(Xor(inds(indPlot))),abs(act(indPlot))*10,cols(indPlot,:),'filled');hold all;axis tight
    end
    drawnow;
    %input('');
end
