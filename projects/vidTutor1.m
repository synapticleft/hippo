...
try
    ...
    while (abortit<2)
        if movieInput
            ...
            fps = realmax;
            movie = Screen('OpenVideoCapture', win);
            Screen('StartVideoCapture', movie, fps, 1);
            tex=Screen('GetCapturedImage', win, movie, 1); 
            imgw = size(tex,2);imgh = size(tex,1);
        end
