function ripChans=rippledet(file,allchan,detchan)%,minuschan)
...
for i = 1:ceil(numSamps/sz)
    ...
    if exist('detchan','var');
        Filtchan = a.data(((inds(1):inds(2))-1)*allchan+detchan);
    else
        Filtchan = mean(reshape(a.data((inds(1)-1)*allchan+1:inds(2)*allchan),[allchan inds(2)-inds(1)+1]));
    end
