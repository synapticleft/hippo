function posTimeSim(pos,v,sp,bounds,gauss)
shift = 1;
...
figure;subplot(212);imagesc(angle(rlAvg(:,indsrl)));colormap hsv;
[~,indslr] = sort(mdscale(pdist([real(lrAvg); imag(lrAvg)]','correlation'),1));
subplot(211);imagesc(angle(lrAvg(:,indslr)));colormap hsv;
figure;subplot(212);imagesc(abs(rlAvg(:,indsrl)));
subplot(211);imagesc(abs(lrAvg(:,indslr)));
return
%axis tight;
figure;subplot(211);plot(real(mean(lrAvg,2)));hold all;plot(-imag(mean(lrAvg,2)));
