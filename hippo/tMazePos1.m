function [allActs posSeq] = tMazePos1(pos,act,refInd)
...
actf = filtLow(abs(act),1250/32,2);
act = abs(act);
%bounded1 = round(resample(bounded,dec,1));
pos = pos(1:size(act,2),:);
bounded1 = interp1(1:numel(bounded),bounded,linspace(1,numel(bounded),size(act,2)));
buffer = round(1250/32*[-15 15]);%[-100 100]/2;
c = jet(range(buffer)+1);
bounded1 = bounded1(1:size(pos,1));
pos = pos(1:numel(bounded1),:);
%bounds([1 3]) = bounds([1 3]) - 10;
%bounds([2 4]) = bounds([2 4]) + 10;
%figure;
tempBound1 = zeros(size(bounded1))';
allActs = zeros(max(bounded1),size(act,1),range(buffer)+1);
for i = 1:max(bounded)-1
%    for j = 1:size(act,1)
        %if max(actf(j,:)) > 3
        tempBound = [find(bounded1 == i,1,'last') find(bounded1 == i+1,1)];
        tempBound1(:) = 0; tempBound1(tempBound(1):tempBound(2)) = 1;
        [~,m] = max(actf(refInd,:).*(tempBound1'));
        inds = m+buffer(1):m+buffer(2);
        %if i == 1 || sign(posSeq(i-1)) ~= sign(posSeq(i))
            try
            allActs(i,:,:) = actf(:,inds);
            catch
                max(inds)
