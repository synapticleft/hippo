...
tic;
link = 'logit';%'identity';
lags = 2;nAhead = 0;%floor(lags/2);
pos(pos == -1) = nan;
%v = diff(pos);
v = angVel(pos);
v = sqrt(v(:,1).^2);%+v(:,2).^2)/2;
v = filtLow(v',1250/32,3)';plot(v);
...
%state = [toeplitz(real(vp11),nan*ones(lags,1)) toeplitz(imag(vp11),nan*ones(lags,1))]; %toeplitz(vp1,nan*ones(lags,1))];
...
out = scale(v(:,1));
train = 1:floor(numel(out)/2);test = (1+floor(numel(out)/2)):numel(out); 
