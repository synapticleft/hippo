function [phi whiteningMatrix] = runTriggerConv1(X,pos,thresh,phi)
%% run convolutional sparse coding on spatioTEMPORALLY whitened data, 
...
[X,whiteningMatrix,dewhiteningMatrix,zerophaseMatrix] = whitenLarge(X,N,R,inds);
reg = reg(inds);
randn('seed',1);
rand('seed',1);
...
f = find(inds);
chunks = 5000;
Xcov = zeros(size(X,1)*R);
Xtoep = zeros(size(X,1)*R,chunks);
...
[V,D] = eig(A);
d = diag(D);
dsqrtinv = real(d + exp(-9)).^(-0.5);
wh = diag(dsqrtinv)*V';
dw = V*diag((d+exp(-10)).^.5);%sqrt(D);
zp = V*wh;
Y = zeros(N,numel(f));
    for i = 1:R
        Y = Y + zp(R/2*size(X,1)+(1:size(X,1)),(i-1)*N+(1:N))*X(:,f-R/2+i);
    end
%wh = wh(end-N+1:end,:);dw = dw(:,end-N+1:end);
%for i = 1:R
%    Y = Y + wh(:,(i-1)*N+(1:N))*X(:,f-R/2+i);%circshift(X,[0 i+R/2]);
%end
toc
clear X;