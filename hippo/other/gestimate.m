function [A W] = gestimate(X, rdim,nsteps)
...
% p.seed = 1;
% p.model = 'tica';
% p.algorithm = 'gradient';%'fixed-point';%
% p.xdim = 7;%rdim;
% p.ydim = 7;%1;
% p.maptype = 'standard';
% p.neighborhood = 'ones3by3';
% %p.groupsize = 2;
% %p.groups = rdim/p.groupsize;
% %p.components = rdim;
% p.stepsize = 0.1;
% p.epsi = 0.005;
p.model     =       'ica';
...
whiteningMatrix = D*Ex';
dewhiteningMatrix = Ex*D^(-1);
...
%     end
%     fprintf(' Done!\n');      
    corrs(i) = min(abs(diag(B' * B1)));%log(1-abs(diag(corr(B,B1))));
end
fprintf('\n');
A = dewhiteningMatrix * B;
W = B' * whiteningMatrix;
figure;plot(log(1-corrs));
