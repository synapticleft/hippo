...
%   toeplitz(abs(vp12),nan*ones(lags,1)) toeplitz(abs(vp11),nan*ones(lags,1))];
...
out = v(:,1);%scale(v(:,1));
train = 1:floor(numel(out)/2);test = (1+floor(numel(out)/2)):numel(out); %1:numel(out);test = 1:numel(out);%
