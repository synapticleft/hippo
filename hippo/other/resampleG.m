function [yGrid] = resample2(yData,oldFs,newFs) % ( xData, yData, xGrid )
...
%xData = (xLim(1):xLim(2))/oldFs;
%xGrid = xData(1):(1/newFs):xData(end);
% Allocate memory for return values.
%yGridGaps = nan(round(size(yData,1)/oldFs*newFs),size(yData,2));
xGrid = max(1,round((1:size(yData,1))/oldFs*newFs))';
yGrid = zeros(round(size(yData,1)/oldFs*newFs),size(yData,2));
for i = 1:size(yData,2)
    yGridGaps = accumarray(xGrid,yData(:,i),[],@mean,nan);
    yGridSelect = ~isnan( yGridGaps );
    %figure;plot(find(yGridSelect))
    %[max(xGrid) size(yGridSelect) sum(ismember(xGrid,find(yGridSelect)))]
    %yGrid(:,i) = interp1( xGrid(ismember(xGrid,find(yGridSelect))), yGrid(yGridSelect), 1:round(size(yData,1)/oldFs*newFs), 'linear', nan(1) );
    yGrid(:,i) = interp1(find(yGridSelect), yGridGaps(yGridSelect), 1:round(size(yData,1)/oldFs*newFs), 'linear', nan(1) );
end
