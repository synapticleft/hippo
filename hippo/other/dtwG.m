function [D,w,Dist,k,rw,tw]=dtwG(r,t,pflag)
%
...
%try norm by snr
d = pdist2(r',t',@myDist);
M=size(r,2);% if (row > M) M=row; r=r'; end;
N=size(t,2);% if (row > N) N=row; t=t'; end;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if pflag
    imagesc(D);hold on;plot(w(:,2),w(:,1),'-w','linewidth',2);hold off;
    return
    % --- Accumulated distance matrix and optimal path
    %figure(1);set(gcf,'Name','DTW - Accumulated distance matrix and optimal path', 'NumberTitle','off');
    ...
    return
    % --- Warped signals
    figure(2);set(gcf,'Name','DTW - warped signals', 'NumberTitle','off');
