function icaSub( X,fname, p )%whiteningMatrix, dewhiteningMatrix, 
...
p
fname = ['/media/work/scripts/hippo/isa/' fname];
...
while 1  
  ...
  if rem(iter,p.write)==0 | iter==1
    ...
    if strcmp(p.algorithm,'gradient')
      eval(['save ' fname ' B p iter obj objiter']);%W A
    else
      eval(['save ' fname ' B p iter']);% W A
    end
    fprintf(' Done!\n');      
