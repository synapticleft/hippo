...
w = watershed(b==0);
w = w-1; %w(w== max(w)) = 0;
%allX = zeros(size(X,1)*(range(win)+1),ceil(size(X,2)/peakToPeak/2));
...
%    runs1 = bwlabel(w>0 & mod(w,2) == k-1 & w <=2*max(runs));
    for i = 1:max(runs1)
        ...
        %oldTheta = angle(-v(runInds));
        oldTheta = angle(-v(runInds)*exp(1i*-1*pi/6));%*-pi/6)
        %[h,pk] = findpeaks(-abs(oldTheta),'minpeakheight',-.3);
        %newTheta = linspace(min(oldTheta),max(oldTheta)
        %h(~inds(runInds(pk))) = [];
        %pk(~inds(runInds(pk))) = [];
