function [posInds,t,vel] = runTriggerViewCb(pos,v,Xf,accumbins,thresh,r,probes,posInds,r1)
...
offSet = 1;
%Xf = [Xf;[zeros(offSet,1);v(1+offSet:end,1)].'];
...
%t = complex(r(:,1:65),r(:,66:end))*conj(zscore(Xf,0,2));%r1*bsxfun(@minus,Xf,mean(Xf,2));%
%t = complex(r(:,1:end/2),r(:,end/2+1:end))*conj(zscore(Xf,0,2));
%t = real(t);%abs(t);%[real(t);imag(t)];
Xf = [[real(Xf);imag(Xf)] [-imag(Xf);-real(Xf)]];
t = r*zscore(Xf,0,2);
t = complex(t(:,1:end/2),t(:,end/2+1:end));
t = abs(t);
%r = complex(r(:,1:end/2),r(:,end/2+1:end));
%t = conj(r)*zscore(Xf,0,2);t = abs(t);
if 0
    [B M] = size(t);
...
%     u = complex(r1(1:size(r1,1)/2,posInds(i)),r1(size(r1,1)/2+1:end,posInds(i)));%r1(1:size(Xf,1)-1,posInds(i));
%     if exist('probes','var') && ~isempty(probes)
%         up1 = probes;
%         for ii = 1:size(probes,1)
%             for j = 1:size(probes,2)
