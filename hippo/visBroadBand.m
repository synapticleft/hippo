...
t = t(:,keepInds(1):keepInds(2));Xf = Xf(:,keepInds(1):keepInds(2));
[B M] = size(t);
...
f1 = figure;subplot(311);sPlot(t1,[],0);
X = getData(fname,range(keepInds)*32,keepInds(1)*32);
X = zscore(X(1:64,:),0,2);
[u,s,v] = svds(X,1);
...
figure(f1);
subplot(312);plot((1:size(X,2))/32,X');hold all;axis tight;%plot(1:size(X,2),s*v','k','linewidth',2);
X = X-u*s*v';
subplot(313);plot((1:size(X,2))/32,X');hold all;plot((1:size(X,2))/32,s*v','k','linewidth',2);axis tight;
