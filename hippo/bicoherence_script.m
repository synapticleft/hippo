%%  MA666 - Lab 10:  CFC and bicoherence
...
%   further explore (e.g., filtering, Hilbert transforms, statistical
...
%   is one for all time.  Make sense?
...
%   and we showed this for a very simple example.  Let's verify that result
...
%   Finally, compute the power spectra of the orginal signal and the
...
%   will define our low and high frequency bands.
%
%   Now, we'll develop a measure to assess the cross frequency coupling.  To do
%   so, we'll implement the three steps discussed in class.
%
...
Wn = [7*2/Fs,10*2/Fs];               %Define the low frequency window of interest.
[B,A] = butter(deg,Wn,'bandpass');  %Apply the filter to isolate the band.
dlo = filtfilt(B,A,d);

Wn = [14*2/Fs,20*2/Fs];            %Define the high frequency window of interest.
[B,A] = butter(deg,Wn,'bandpass');  %Apply the filter to isolate the band.
dhi = filtfilt(B,A,d);
