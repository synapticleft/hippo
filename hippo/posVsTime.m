...
%pos = angle(-complex(pos(:,1)-170,pos(:,2)-120));
...
for i = 1:max(lrRuns)
    ...
    %plot(pos(inds,1),pos(inds,2));hold all;plot(binsTimes,lrTimes(:,i));hold off;pause(.1);
    %scatter(binsTimes,lrTimes(:,i)+i,20*abs(lrAvg(:,i)),angCol(phase2Col(angle(lrAvg(:,i))),:),'filled');
end
%drawnow;
