function [spT spId] = spikePlots(file,pos,v,spT,spId,spf)%,sp)
...
nanInds = isnan(pos(:,1));
pos = pos(~nanInds,:);v = v(~nanInds,:);spf = spf(:,~nanInds);
...
%for i = 1:max(runs)
 ...
 if numel(tempTimes) > 100
        ...
        figure(1);scatter(weighted(tempTimes,pos(:,1)),weighted(tempTimes,(1:size(pos,1))'),30,angCol(phase2Col(angle(weighted(tempTimes,spf(spSort(j),:).'))),:),'filled');
       ...
       %h = bsxfun(@rdivide,h,v2Hist);
        figure(2);subplot(211);imagesc(sqrt(h));
        subplot(212);imagesc(sqrt(h1));
