function [E, D, cumVar] = gpcamat(vectors, firstEig, lastEig, s_interactive, s_verbose);
...
rankTolerance = 1e-7;
%maxLastEig = sum (diag (D) > rankTolerance); original
d = flipud(diag(D));
cumVar = sum(d);
%figure;plot(log(d));drawnow;
maxLastEig = sum(cumsum(d)/cumVar < .99999)
